...
export default function StudentSubjectDashboard() {
  ...
  async function loadDashboard() {
    ...
    // 👇 Login page StudentDashboard jaisa hi pattern: sessionStorage se "user" nikalo
    ...
    /* Attendance (sirf is student ki, is subject ki) */
    ...
    /* Marks (sirf is student ki, is subject ki) */
    ...
    if (marks && marks.length > 0) {
      const total = marks.reduce(
        (sum: number, item: any) => sum + Number(item.total_marks ?? 0),
        0
      );

      setAverageMarks(Number((total / marks.length).toFixed(2)));
    } else {
      setAverageMarks(0);
    }

    /* Assignments (subject-wide count) */
    ...
    /* Announcements (subject-wide count) */
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100">
      <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-6 md:px-8 md:py-8">
        ...
        >
          ...
          <div className="mt-6 sm:mt-8 md:mt-10 grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-4 md:gap-5">
            <div className="rounded-xl sm:rounded-2xl bg-white/15 p-3 sm:p-4 md:p-5 backdrop-blur-md">
              <p className="text-xs sm:text-sm text-blue-100">Subject Type</p>
            ...
            <div className="rounded-xl sm:rounded-2xl bg-white/15 p-3 sm:p-4 md:p-5 backdrop-blur-md">
              <p className="text-xs sm:text-sm text-blue-100">Credits</p>
              <h3 className="mt-1 sm:mt-2 text-lg sm:text-xl md:text-2xl font-bold">
                {subject.credits}
              </h3>
            </div>
            ...
            <div className="rounded-xl sm:rounded-2xl bg-white/15 p-3 sm:p-4 md:p-5 backdrop-blur-md">
              <p className="text-xs sm:text-sm text-blue-100">Attendance</p>
              <h3 className="mt-1 sm:mt-2 text-lg sm:text-xl md:text-2xl font-bold">
                {attendancePercent}%
              </h3>
            </div>
            ...
            <div className="rounded-xl sm:rounded-2xl bg-white/15 p-3 sm:p-4 md:p-5 backdrop-blur-md">
              <p className="text-xs sm:text-sm text-blue-100">Average Marks</p>
              <h3 className="mt-1 sm:mt-2 text-lg sm:text-xl md:text-2xl font-bold">
                {averageMarks}
              </h3>
            </div>
        ...
        >
          {/* Attendance */}
          <div
            onClick={() => router.push(`/students/attendance/${subject.id}`)}
          ...
          >
            <ClipboardCheck className="h-8 w-8 sm:h-10 sm:w-10 md:h-[50px] md:w-[50px] transition-transform duration-300 group-hover:rotate-12" />
          ...
          {subject.subject_type === "Theory" && (
            <>
              {/* Marks */}
              <div
                onClick={() => router.push(`/students/marks/${subject.id}`)}
              ...
              >
                <BarChart3 className="h-8 w-8 sm:h-10 sm:w-10 md:h-[50px] md:w-[50px] transition-transform duration-300 group-hover:rotate-12" />
                <h2 className="mt-3 sm:mt-4 md:mt-6 text-lg sm:text-xl md:text-2xl font-bold">
                  Marks
                </h2>
              ...
              {/* Assignments */}
              <div
                onClick={() => router.push(`/students/assignments/${subject.id}`)}
              ...
              >
                <FileText className="h-8 w-8 sm:h-10 sm:w-10 md:h-[50px] md:w-[50px] transition-transform duration-300 group-hover:rotate-12" />
                <h2 className="mt-3 sm:mt-4 md:mt-6 text-lg sm:text-xl md:text-2xl font-bold">
                  Assignments
                </h2>
          ...
          {/* Announcements */}
          <div
            onClick={() => router.push(`/students/announcements/${subject.id}`)}
          ...
          >
            <Bell className="h-8 w-8 sm:h-10 sm:w-10 md:h-[50px] md:w-[50px] transition-transform duration-300 group-hover:rotate-12" />
            <h2 className="mt-3 sm:mt-4 md:mt-6 text-lg sm:text-xl md:text-2xl font-bold">
              Announcements
            </h2>
        ...
        <div
          className={`hidden md:grid gap-6 mt-10 ${
            subject.subject_type === "Theory" ? "lg:grid-cols-3" : "lg:grid-cols-2"
          }`}
        >
          <div className="rounded-3xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <ClipboardCheck size={42} className="text-green-600" />
            </div>
          </div>
        ...
        <div className="hidden md:block mt-10 rounded-3xl bg-white p-8 shadow-xl">
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <h3 className="font-semibold">{subject.courses?.course_name}</h3>
            </div>

            <div>
              <p className="text-gray-500">Semester</p>
              <h3 className="font-semibold">{subject.semesters?.semester_no}</h3>
            </div>
