...
export default function TeacherMarksPage() {
  ...
  async function loadStudents(
    ...
    // 1) Fetch all students for this dept/course/semester in one query
    ...
    // 2) Fetch ALL marks rows for these students + this subject in ONE query
    ...
    // 3) Build a lookup map so we can match marks to students in memory (fast)
    const marksMap = new Map<string, any>();
    for (const mark of marksData) {
      marksMap.set(mark.student_id, mark);
    }

    const rows: Student[] = studentsData.map((student) => {
  ...
  async function saveAllMarks() {
    try {
      ...
      // Single batched upsert instead of one select + one insert/update
      // PER STUDENT. Requires a unique constraint on (student_id, subject_id)
      // in the `marks` table for onConflict to work correctly.
